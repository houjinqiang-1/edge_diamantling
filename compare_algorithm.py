...
def draw_anc(G1, G2, MCC1, MCC2,  num):
    ...
    for i in range(0, len(MCC1)):
        x1[i] = i / len(MCC1)
    ratio_score = [sum(i / (len(G1.edges()) + len(G2.edges())) for i in MCC1)]
    ...
    x2 = np.ones(len(MCC2))
    find_score = [sum(i / (len(G1.edges()) + len(G2.edges())) for i in MCC2)]
    for i in range(0, len(MCC2)):
        x2[i] = i / len(MCC2)

    plt.figure(figsize=(10,5))
    plt.plot(x1, MCC1, label=f'ratiocut_anc:{ratio_score}')
    plt.plot(x2, MCC2, label=f'finder_anc:{find_score}')
    plt.ylabel('Value')
    plt.title(f'{num}_nodes')
    plt.legend()
    # 设置 x 轴范围为 0 到 0.2
    plt.xlim(0, 1)
    plt.savefig(f'./compare_random/{num}_ratio_best.png')
...
if __name__ == "__main__" :
    ...
    node_num = 30
    adj1 = np.load(f"./data/syn_30/{node_num}/adj1_%s.npy" % 100)
    adj2 = np.load(f"./data/syn_30/{node_num}/adj2_%s.npy" % 100)
    ...
    for i in range(len(G1)):
        for j in G1[i]:
            if i < j:
                merge_graph[0].append(ind)
                ind += 1
                merge_graph[1].append((i, j))
    for i in range(len(G2)):
        for j in G2[i]:
            if i < j:
                ...
                merge_graph[1].append((i, j))
    edge_sequence = []
    with open('../Generalized-Network-Dismantling-master/ratiocut_sequence.txt', 'r') as file:
        for j in file:
            j = eval(j)
            edge_sequence.append(j)

    merge_graph_dic = {}
    for j in merge_graph[0]:
        merge_graph_dic.setdefault(j, merge_graph[1][j])

    G1, G2, M_original_max= MCC(G1, G2)
    M_ori = M_original_max
    sequence = []
    for edge_1, edge_2 in G1.edges:
        if (edge_1, edge_2) in merge_graph[1]:
            keys = list(merge_graph_dic.keys())[list(merge_graph_dic.values()).index((edge_1, edge_2))]
    ...
    for edge_1, edge_2 in G2.edges:
        if (edge_1, edge_2) in merge_graph[1]:
            ...
            if keys >= original_edge:
                sequence.append(keys)
    merge_graph[0] = sequence
    ratio_MaxCClist = []


    for index_1 in edge_sequence:
        if index_1 in merge_graph[0]:
            if index_1 < original_edge:
                G1.remove_edge(merge_graph[1][index_1][0], merge_graph[1][index_1][1])
            else:
                G2.remove_edge(merge_graph[1][index_1][0], merge_graph[1][index_1][1])
            ...
            ratio_MaxCClist.append(M_ori/M_original_max)
    ...
    draw_anc(G1, G2, ratio_MaxCClist, finder_MaxCClist, node_num)
